...
<div class="lg:flex justify-between mt-20 items-center">
    <div data-aos="fade-right" data-aos-delay="200">
        <h1 class="text-white md:text-left uppercase mb-12 text-center">
            <span class="text-6xl md:text-7xl font-bold tracking-widest block">{{ getSetting('organizationName')
                }}</span>
            <span class="text-2xl md:text-3xl tracking-wider font-light block">{{ getSetting('organizationUniversity')
                }}</span>
        </h1>
...
<div class="container mx-auto px-6 lg:px-28 py-6 mt-28">
    <div class="lg:flex justify-between items-center">
        <img src="{{ asset('assets/images/about-himatif.png') }}" class="mr-24 hidden md:block w-1/2"
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-16">
                ...
                <div class="mb-12 border-2 border-gray-200 rounded-xl p-2 hover:shadow-md" data-aos="zoom-in-up">
                    ...
                    <div class="text-gray-800 text-lg font-bold mt-3 px-4">
                        <a href="{{ route('blog.post', ['post' => $post->id, 'slug' => $post->slug]) }}">{{ $post->title
                            }}</a>
                    </div>
    ...
    <div class="mt-36">
        ...
        <div class="grid grid-row md:grid-cols-2 lg:grid-cols-3 place-items-center" data-aos="fade-up"
            ...
            <div class="border-2 cursor-pointer hover:border-gray-400 transition duration-500 ease-in-out w-64 h-64 lg:w-80 lg:h-80 rounded-3xl p-5 mb-5 flex flex-wrap content-center
                        @if (isset($headOfDivisions[$item->id])) modal-open @endif" data-division-id="{{ $item->id }}">
                @isset($item->media[0])
    ...
    <div class="modal-container bg-white w-full md:w-4/6 mx-auto rounded shadow-lg z-50 overflow-y-auto">
        ...
        <div class="modal-content py-12 text-left px-6 md:px-12 overflow-auto" style="height: 80vh;">
            ...
            <div class="w-full flex flex-col items-center mb-6">
                <span class="block font-poppins font-semibold text-xl text-gray-700">{{
                    $headOfDivisions[$divisionMember[0]['position']->division->id][0]->user->name }}</span>
                <span class="block font-poppins text-lg text-gray-500">{{
                    $headOfDivisions[$divisionMember[0]['position']->division->id][0]->user->member->npm }}</span>
            ...
            <div
                class="mb-3 flex items-center justify-between px-7 w-full h-12 border-2 border-gray-300 rounded-lg bg-gray-50">
                <span class="font-poppins text-sm md:text-base font-medium text-gray-800">{{ $member->user->name
                    }}</span>
